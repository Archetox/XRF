    ...
    with open(countfile) as g:  # Opens count file. Structure of count file is rows = energy,
        for line in g:  # columns = counts for each x,y position.
            ...
            for i in range(0, yno,
                           1):  # Loops through counts and populates 3D array, filling each column of a row first
                for j in range(0, xno,
                               1):  # then moves to next row and fills columns until all counts are assigned to the
                    ecounts[i, j, n] = float(
                        a[z])  # 2D matrix. The energy value is then changed and the next 2D matrix populated
                    z = z + 1
...
def main():
    ...
    ecounts = deccount(countfile, xno, yno,
                       eno)  # Obtain 3D array filled with detector counts for x,y positions at each energy value
    ...
    #
    dset11 = h5file.create_dataset("/xrfmap/positions/name", (2,),
                                   dtype="S20")  # Create new dataset with type string (20 characters)
    dset16 = h5file.create_dataset("/xrfmap/scalers/name", (1,),
                                   dtype="S20")  # Create new dataset with type string (20 characters)

    scalarname = []  # Creating a list in which headings will go
    scalarname.append(b"MiniIon")
    dset16[...] = scalarname  # Populating the dataset with heading names

    posname = []  # Creating a list in which headings will go
    posname.append(b"x_pos")
    posname.append(b"y_pos")
